...
def greedyDecode(model,encoder,revcoder,decoder,encoder_params,decoder_params,sentence_de,downstream=False):
    dy.renew_cg()
...
def do_one_example(model,encoder,revcoder,decoder,encoder_params,decoder_params,sentence_de,sentence_en,downstream=False):
    dy.renew_cg()
    total_words=len(sentence_en)
    encoder_lookup=encoder_params["lookup"]
    decoder_lookup=decoder_params["lookup"]
...
sentences_de,sentences_en,wids=readData.getData()

reverse_wids=readData.reverseDictionary(wids)

train_sentences_de=sentences_de[:700]
train_sentences_en=sentences_en[:700]

print len(train_sentences_de)
print len(train_sentences_en)

valid_sentences_de=sentences_de[700:]
valid_sentences_en=sentences_en[700:]
...
for epochId in xrange(NUM_EPOCHS):    
    ...
    for sentenceId,sentence in enumerate(train_sentences):
        ...
        loss,words=do_one_example(model,encoder,revcoder,decoder,encoder_params,decoder_params,sentence_de,sentence_en,downstream=True)
        loss.value()
        loss.backward()
        trainer.update()
        if totalSentences%100==0:
            ...
            for valid_sentence in valid_sentences:
                ...
                validLoss,words=do_one_example(model,encoder,revcoder,decoder,encoder_params,decoder_params,valid_sentence_de,valid_sentence_en,downstream=True)
